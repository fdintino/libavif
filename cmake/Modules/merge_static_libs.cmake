    ...
    foreach(lib ${args})
        if(TARGET "${lib}")
            ...
            if(${target_type} STREQUAL "STATIC_LIBRARY")
                ...
                if(${lib} STREQUAL "avif_internal" OR is_avif_local)
                    list(APPEND libs $<TARGET_FILE:${lib}>)
                    list(APPEND dependencies "${lib}")
                endif()
                avif_collect_deps(${lib} lib_deps)
                foreach(child_lib ${lib_deps})
                    get_target_property(child_target_type ${child_lib} TYPE)
                    if(${child_target_type} STREQUAL "INTERFACE_LIBRARY")
                        continue()
                    endif()
                    get_target_property(is_avif_local ${child_lib} AVIF_LOCAL)
                    if(is_avif_local)
                        list(APPEND libs $<TARGET_FILE:${child_lib}>)
                        list(APPEND dependencies "${child_lib}")
                    endif()
                endforeach()
            endif()
        endif()
